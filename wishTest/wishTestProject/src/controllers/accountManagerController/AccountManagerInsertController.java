...
public class AccountManagerInsertController implements SubController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) 
		...
		if(resultCustom == 1) {
			System.out.println("Custom 데이터 저장 완료 - 값 : " + resultCustom);
		...
		if(resultAccount == 1) {
			System.out.println("Account 데이터 저장 완료 - 값 : " + resultAccount);
	...
	private CustomVO getParamForCustomVO(HttpServletRequest request) {
		...
		if(voContractPeriod_E_String != null && voContractPeriod_E_String.length() > 0) {
			LocalDate voContractPeriod_E = Date.valueOf(voContractPeriod_E_String).toLocalDate();
			if(voContractPeriod_E != null) {
				customVO.setVoContractPeriod_E(voContractPeriod_E);
			}
		}
